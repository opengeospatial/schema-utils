...
/**
 * A SAX and DOM error handler that collects validation errors raised while verifying the
...
public class ValidationErrorHandler implements ErrorHandler, DOMErrorHandler, Iterable<ValidationError> {
	 ...
	 * {@inheritDoc}
	 *
	 * Receives notification of a recoverable error. Typically this indicates that a
	 ...
	 * {@inheritDoc}
	 *
	 * Receives notification of a non-recoverable error, such as non-XML content, XML that
	 * is not well-formed, or an incorrect encoding declaration.
	...
	private void addSAXError(ErrorSeverity severity, SAXParseException spex) {
		...
		ValidationError error = new ValidationError(severity, spex.getMessage(), spex.getLineNumber(),
	 ...
	 * {@code DOMError}.
	 * @param severity The severity of the error.
	 * @param err The <code>DOMError</code> raised while validating the XML source
	 * document.
	 */
	...
	public boolean handleError(DOMError error) {
		switch (error.getSeverity()) {
			case DOMError.SEVERITY_WARNING:
				addDOMError(ErrorSeverity.WARNING, error.getMessage(), error.getLocation());
				break;
			case DOMError.SEVERITY_ERROR:
				addDOMError(ErrorSeverity.ERROR, error.getMessage(), error.getLocation());
				break;
			case DOMError.SEVERITY_FATAL_ERROR:
				addDOMError(ErrorSeverity.CRITICAL, error.getMessage(), error.getLocation());
				break;
		}
	 ...
	 * @param locator An <code>ErrorLocator</code> instance that provides information
	 * regarding the location of the error.
	 */
	 ...
	 * @param errors A collection of <code>ValidationError</code> objects, each of which
	 * encapsulates information about some constraint violation (which are not necessarily
	 * related).
	 */
	...
	/**
	 * <p>
	 * Returns a simple XML representation of all received errors. The structure is shown
	 * below:
	 * </p>
	 *
	 * <pre>
	...
	/**
	 * <p>
	 * Getter for the field <code>schemaLanguage</code>.
	 * </p>
	...
	/**
	 * <p>
	 * Setter for the field <code>schemaLanguage</code>.
	 * </p>
	 * @param schemaLanguage a {@link java.lang.String} object
	 */
