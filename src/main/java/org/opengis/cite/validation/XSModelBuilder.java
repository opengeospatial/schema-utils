...
/**
 * Provides methods for building schema models that can be used to access and query the
 * post-schema-validation infoset (PSVI). It employs the XML Schema API implemented by the
 * Apache Xerces2 parser.
 *
 * @see <a href="http://xerces.apache.org/xerces2-j/javadocs/xs/index.html" target=
 * "_blank">XML Schema API</a>
 */
public class XSModelBuilder {

	/**
	 * <p>
	 * Constructor for XSModelBuilder.
	 * </p>
	 */
	 ...
	 * @param xsd A Schema representing an XML Schema resource.
	 * @param targetNamespace The target namespace of a schema (an absolute URI).
	...
	public static XSModel buildXMLSchemaModel(Schema xsd, String targetNamespace) {
		...
		if (XSGrammarPoolContainer.class.isAssignableFrom(xsd.getClass())) {
			XSGrammarPoolContainer poolContainer = XSGrammarPoolContainer.class.cast(xsd);
			XMLGrammarPool pool = poolContainer.getGrammarPool();
			grammars = pool.retrieveInitialGrammarSet(XMLConstants.W3C_XML_SCHEMA_NS_URI);
		}
		XSModel xsModel = null;
		for (int i = 0; i < grammars.length; i++) {
			XSGrammar xsGrammar = (XSGrammar) grammars[i];
			if (xsGrammar.getGrammarDescription().getNamespace().equals(targetNamespace)) {
