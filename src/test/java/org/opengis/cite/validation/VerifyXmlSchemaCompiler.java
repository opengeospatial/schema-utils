...
public class VerifyXmlSchemaCompiler {
	...
	public static void setUpClass() throws Exception {
		entityCatalog = VerifyXmlSchemaCompiler.class.getResource("/entity-catalog.xml");
	...
	public void incompleteSchema() throws SAXException, IOException {
		File schemaFile = new File(TEST_RESOURCES + "xsd/shiporder-incomplete.xsd");
		XmlSchemaCompiler xsdCompiler = new XmlSchemaCompiler(entityCatalog);
		Schema schema = xsdCompiler.compileXmlSchema(schemaFile.toURI());
		assertNotNull("Schema is null", schema);
		ValidationErrorHandler err = xsdCompiler.getErrorHandler();
		assertEquals("Unexpected number of compilation errors", 1, err.getErrorCount());
	}
	...
	public void invalidSchema() throws SAXException, IOException {
		...
		assertNotNull("Schema is null", schema);
		ValidationErrorHandler err = xsdCompiler.getErrorHandler();
		assertEquals("Unexpected number of compilation errors", 2, err.getErrorCount());
	}
	...
	public void schemaFromDOMSource() throws SAXException, IOException {
		...
		XmlSchemaCompiler xsdCompiler = new XmlSchemaCompiler(entityCatalog);
		Schema schema = xsdCompiler.compileXmlSchema(new DOMSource(doc, doc.getDocumentURI()));
		assertNotNull("Schema is null", schema);
		ValidationErrorHandler err = xsdCompiler.getErrorHandler();
		assertFalse("Compilation errors were reported.", err.errorsDetected());
	}
	...
	@Test
	@Ignore("Passes, but avoid establishing a network connection")
	public void compileSchema_includeHasRelativeURI() throws SAXException, IOException {
		URI schemaURI = URI.create("http://www.oracle.com/webfolder/technetwork/jsc/xml/ns/javaee/javaee_7.xsd");
		XmlSchemaCompiler xsdCompiler = new XmlSchemaCompiler(entityCatalog);
		Schema schema = xsdCompiler.compileXmlSchema(schemaURI);
		assertNotNull("Schema is null", schema);
		ValidationErrorHandler err = xsdCompiler.getErrorHandler();
