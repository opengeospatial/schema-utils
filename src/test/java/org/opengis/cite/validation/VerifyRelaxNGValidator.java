...
public class VerifyRelaxNGValidator {
	...
	public static void setUpClass() throws Exception {
		...
		doc = docBuilder.parse(new File(TEST_RESOURCES + "holiday-missingDay.xml"));
	}
	...
	@Test
	public void buildValidatorWithoutSchemaShouldFail() throws SAXException, IOException {
	...
	@Test
	public void buildValidatorUsingXmlGrammar() throws SAXException, IOException {
	...
	@Test
	public void buildValidatorUsingCompactGrammar() throws SAXException, IOException {
	...
	public void validateHoliday_missingDay() throws Exception {
		...
		rngValidator.validate(new StreamSource(new File(TEST_RESOURCES + "holiday-missingDay.xml")));
		ValidationErrorHandler err = rngValidator.getErrorHandler();
		Assert.assertEquals("Unexpected number of errors.", 1, err.getErrorCount());
	}
	...
	public void validateHoliday_ok() throws Exception {
		URL schemaRef = getClass().getResource("/relax/publicHoliday.rng");
		RelaxNGValidator rngValidator = new RelaxNGValidator(schemaRef);
		rngValidator.validate(new StreamSource(new File(TEST_RESOURCES + "holiday.xml")));
		ValidationErrorHandler err = rngValidator.getErrorHandler();
		Assert.assertEquals("Unexpected number of errors.", 0, err.getErrorCount());
	}
	...
	public void validateSVRLReport() throws Exception {
		...
		Assert.assertEquals("Unexpected number of errors.", 2, err.getErrorCount());
		String expectedErrMessage = "attribute \"document\" not allowed";
		String errMessages = err.toString();
		Assert.assertTrue("Expected error message to contain: " + expectedErrMessage,
				errMessages.contains(expectedErrMessage));
	}

	@Test(expected = SAXException.class)
	public void createValidatorUsingInvalidGrammar() throws SAXException, IOException {
	...
	@Test
	public void validateAgainstMergedCompactGrammar() throws SAXException, IOException {
